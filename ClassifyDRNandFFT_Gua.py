...
class GaussianHighPass2D(nn.Module):
    """
    ...
    def __init__(self, D0=30.0):
        ...
        ----------
        D0 : float
            Cutoff frequency (radius in frequency domain).
        """
        super(GaussianHighPass2D, self).__init__()
        self.D0 = float(D0)

        # cache filter (not saved in state_dict)
        self.register_buffer("H_filter", None, persistent=False)

    def _build_filter(self, H, W, device, dtype):
        """
        Build (or reuse) Gaussian high-pass filter for given spatial size.
        """
        ...
        # Gaussian High-Pass
        Hf = 1.0 - torch.exp(-D2 / (2.0 * (self.D0 ** 2)))

        # Match unshifted FFT convention
        Hf = torch.fft.ifftshift(Hf)

        # Shape: (1, 1, H, W) for broadcasting
        Hf = Hf.unsqueeze(0).unsqueeze(0)
        self.H_filter = Hf
        return Hf
    ...
    def forward(self, x):
        ...
        -------
        torch.Tensor
            Gaussian high-pass filtered tensor
        """
        ...
        # Build / reuse filter
        Hf = self._build_filter(H, W, device, dtype)
...
class DRNWithButterworth(nn.Module):
    ...
    def __init__(self, base_model, D0=30.0, n=2):
        ...
        self.hpf = GaussianHighPass2D(D0=D0, n=n)
